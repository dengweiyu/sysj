...
public class RecordingManager implements RecordingOperation {
    ...
    private void _initialize(Context context) {
        ...
        if (context == null) {
            sendMessage(Contant.Msg.CONTEXT_IS_NULL);
            return;
        }
    ...
    public void startScreenCapture(String path) {
        ...
        if (Build.VERSION.SDK_INT < 21 &&
                !Utils.root()) {
            sendMessage(Contant.Msg.ROOT_IS_INVALID);
            return;
        }
        this.path = path;
        _startCapture(path);
    ...
    public void startScreenRecord(String path, Configuration configuration) {
        ...
        if (isRecording.get() == true) {
            sendMessage(Contant.Msg.STATE_RECORDING);
            return;
        }
        this.path = path;
    ...
    public void stopScreenRecord() {
        ...
        if (isRecording.get() == false) {
            sendMessage(Contant.Msg.STATE_COMPLATED);
            return;
        }
    ...
    private Handler handler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case Contant.Msg.CONTEXT_IS_NULL:
                    isRecording.set(false);
                    path = null;
                    configuration = null;
                ...
                case Contant.Msg.PATH_IS_INVALID:
                    isRecording.set(false);
                    path = null;
                    configuration = null;
                ...
                case Contant.Msg.API44_DPWN_HAS_NOT_PAUSE:
                    isPausing.set(false);
                    Log.d(TAG, "handler: API44_DPWN_HAS_NOT_PAUSE");
                    break;

                case Contant.Msg.STATE_RECORDING:
                    isRecording.set(true);
                    isPausing.set(false);
                    Log.d(TAG, "handler: STATE_RECORDING");
                    break;

                case Contant.Msg.STATE_PAUSE:
                    isPausing.set(true);
                    Log.d(TAG, "handler: STATE_PAUSE");
                    break;

                case Contant.Msg.STATE_TIMEING:
                    gc();
                    break;

                case Contant.Msg.STATE_COMPLATED:
                    isRecording.set(false);
                    path = null;
                    configuration = null;
    ...
    public void startTiming() {
        ...
        secs.set(0);
        ++maxMillis;
        sendMessage(Contant.Msg.STATE_TIMEING);
    ...
    private void startRecord(String path, Configuration configuration) {
        ...
        if (path == null || path.length() == 0) {
            ...
            sendMessage(Contant.Msg.PATH_IS_INVALID);
            return;
        }
        File file = null;
        try {
        ...
        if (Build.VERSION.SDK_INT >= 21) {// 5.0以上
            ...
            // ScreenRecordActivityb._startRecording(path, configuration);
            ...
            h.post(new Runnable() {
                ...
                public void run() {
                    ...
                    ScreenRecordActivityb.startRecording(context, p, c);
                }
            });
    ...
    private void pauseRecord() {
        ...
        sendMessage(Contant.Msg.STATE_PAUSE);
        if (Build.VERSION.SDK_INT >= 21) {// 5.0以上
            ...
            sendMessage(Contant.Msg.API44_DPWN_HAS_NOT_PAUSE);
        }
    }
