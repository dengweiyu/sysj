...
//					ScreenRecordActivity.handler.sendEmptyMessage(1);
                    ScreenRecordActivity.sendEmptyMessage(1);
...
//					ScreenRecordActivity.handler.sendEmptyMessage(2);
                    ScreenRecordActivity.sendEmptyMessage(2);
				} else if (msg.equals("xiaomi")) {
					// 检测后台ROOT权限是否开启
					if (Helpers.getRootNotify("xiaomi", mContext)) {
						if (getManufacturer().equals("MIUI")) {
							rootNote = "您的系统是MIUI，请保证您的系统是小米的开发版系统" + "\n"
									+ "然后到“安全中心》授权管理》root权限管理》录屏大师》授权”" + "\n"
									+ "再到“安全中心》应用权限管理》录屏大师》悬浮窗权限》允许”即可正常使用";
...
//							ScreenRecordActivity.handler.sendEmptyMessage(1);
					...
					// 检测后台ROOT权限是否开启
					if (Helpers.getRootNotify("OPPO", mContext)) {
...
//					ScreenRecordActivity.handler.sendEmptyMessage(2);
	...
	public void createCustomDialog(Context context) {
		...
		View view = LayoutInflater.from(context).inflate(
				R.layout.fm_get_root_tool, null);
	...
	public void tipManufacturer() {
		...
		if (getManufacturer().equals("HUAWEI")) {
								...
								public void onClick(DialogInterface dialog,
									...
									try {
										...
										try {
                                            ...
                                            ToastHelper.s("抱歉，您当前的系统版本需要手动打开浮窗");
										}
									}
			...
			new AlertDialog.Builder(mContext).setTitle("温馨提示").setMessage(note)
