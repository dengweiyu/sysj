...
/**
 * User: Ryan Date: 11-10-12 Time: Afternoon 2:30
 */
public class RUtils {
	...
	static {
		mContent = AppManager.getInstance().getApplication();
	}
	...
	public static int getIdentifier(String name, String type) {
		...
		if (null != res) {
			...
			Context context = RUtils.mContent;
			try {
				if ("styleable".equals(type)) {
					res = (Integer) getRStyleableClz(context).getDeclaredField(
							name).get("");
				} else if ("style".equals(type)) {
					res = (Integer) getRStyleClz(context)
							.getDeclaredField(name).get("");
				} else {
					res = context.getResources().getIdentifier(name, type,
			...
			if (res > -1) {
				...
				throw new RuntimeException(
						"res id not found in project, type :" + type
								+ " name :" + name);
			}
		}
	}
	...
	public static int[] getRStyleableIDs(String name) {
		try {
			return (int[]) getRStyleableClz(mContent).getDeclaredField(name)
					.get("");
