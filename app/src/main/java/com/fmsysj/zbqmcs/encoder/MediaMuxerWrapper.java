package com.fmsysj.zbqmcs.encoder;
...
public class MediaMuxerWrapper {
	...
	/** 时间戳的缩减量 */
	static long ptsUsecDec = 0;
	/** 暂停时的时间戳 */
	static long ptsUsecPause = 0;
	/** 重启时的时间戳 */
	static long ptsUsecRestart = 0;
	...
	public MediaMuxerWrapper(String videoFileName) throws IOException {
		mMediaMuxer = new MediaMuxer(videoFileName,
	...
	/**
	 * request startCollectionActivity recording from encoder
	 * 
	...
	synchronized boolean start() {
		if (DEBUG)
			Log.v(TAG, "startCollectionActivity:");
		mStatredCount++;
	...
	/**
	 * request stopRequestService recording from encoder when encoder received EOS
	 */
	/* package */@SuppressLint("NewApi")
	synchronized void stop() {
		if (DEBUG)
			Log.v(TAG, "stopRequestService:mStatredCount=" + mStatredCount);
		mStatredCount--;
	...
	public static final File getCaptureFile(final String type, final String ext) {
		final File dir = new File(
				Environment.getExternalStoragePublicDirectory(type), DIR_NAME);
