package com.fmsysj.zbqmcs.frontcamera;
...
import com.fmsysj.zbqmcs.imageFilters.SoftGlowFilter;
...
public class CameraView extends LinearLayout implements SurfaceHolder.Callback,
	...
	public CameraView(Context context, AttributeSet attrs) {
		...
		inflater.inflate(R.layout.fm_front_camera_view, this);
		...
		if (mCamera != null && !bIfPreview) {
			...
			if (null != supportedPreviewSizes
				...
				for (int i = 0; i < supportedPreviewSizes.size(); i++) {
					size = (Camera.Size) supportedPreviewSizes.get(i);
	...
	public void onPreviewFrame(byte[] data, Camera camera) {
		...
		synchronized (surfaceholder) {
			if (data != null && isClose == false) {
				...
				// ExApplication.cameraRunning = true;
