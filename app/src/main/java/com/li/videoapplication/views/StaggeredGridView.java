...
public class StaggeredGridView extends ExtendableListView {
    ...
    /**
     * A key-value collection where the key is the checkPosition and the
     * {@link GridItemRecord} with some info about that checkPosition
     * so we can maintain it's checkPosition - and reorg on orientation change.
     */
    ...
    public StaggeredGridView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        // Creating these tipEmpty arrays to avoid saving null states
    ...
    // Grid padding is applied to the data item rows but not the header and footer
    ...
    protected void onMeasureChild(final View child, final LayoutParams layoutParams) {
        ...
        if (viewType == ITEM_VIEW_TYPE_HEADER_OR_FOOTER ||
            ...
            if (DBG) Log.d(TAG, "onMeasureChild BEFORE checkPosition:" + position +
                    " h:" + getMeasuredHeight());
        ...
        if (DBG) Log.d(TAG, "onMeasureChild AFTER checkPosition:" + position +
                " h:" + childHeight);
    }
    ...
    protected void onChildCreated(final int position, final boolean flowDown) {
        ...
        if (!isHeaderOrFooter(position)) {
            // do we already have a column for this checkPosition?
            final int column = getChildColumn(position, flowDown);
            setPositionColumn(position, column);
            if (DBG) Log.d(TAG, "onChildCreated checkPosition:" + position +
                                " is in column:" + column);
        }
        else {
    ...
    private void layoutGridChild(final View child, final int position,
        ...
        if (DBG) Log.d(TAG, "onLayoutChild checkPosition:" + position +
                " column:" + column +
    ...
    protected void onOffsetChild(final View child, final int position,
        ...
        // we still want to imageupload_add its deets into our store
        if (isHeaderOrFooter(position)) {
    ...
    private void offsetGridChild(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childTop) {
        ...
        if (DBG) Log.d(TAG, "onOffsetChild checkPosition:" + position +
                " column:" + column +
                " childTop:" + childTop +
                " gridChildTop:" + gridChildTop +
                " gridChildBottom:" + gridChildBottom);
     ...
     * layout the that checkPosition and then fillUp and fillDown appropriately.
     */
    private void onColumnSync() {
        ...
        // re-calc our relative checkPosition while at the same time
        ...
        for (int pos = 0; pos < syncPosition; pos++) {
            ...
            if (DBG) Log.d(TAG, "onColumnSync checkPosition:" + pos +
                                " top:" + top +
                                " bottom:" + bottom +
                                " height:" + height +
                                " heightRatio:" + heightRatio);
        ...
        // our sync checkPosition will be displayed in this column
        final int syncColumn = getHighestPositionedBottomColumn();
        setPositionColumn(syncPosition, syncColumn);

        // we want to offset from height of the sync checkPosition
        // minus the offset
    ...
    private void setPositionHeightRatio(final int position, final int height) {
        ...
        if (DBG) Log.d(TAG, "checkPosition:" + position +
                            " width:" + mColumnWidth +
                            " height:" + height +
    ...
    private int getChildColumn(final int position, final boolean flowDown) {
        ...
        // do we already have a column for this child checkPosition?
        int column = getPositionColumn(position);
    ...
    /**
     * Extended LayoutParams to column checkPosition and anything else we may been for the grid
     */
