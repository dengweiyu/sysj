 ...
 * 设置指示器长度{@link #setIndicatorLength(int)}
 * </pre>
 */
...
//	private Paint mDividerPaint;//分割线的画笔
	 ...
	 * 指示器长度为tab的总长度
	 */
	public static final int INDICATOR_LENGTH_MODE_1 = 1;
	/**
	 * 指示器长度为tab的总长度的1/2
	 */
	public static final int INDICATOR_LENGTH_MODE_2 = 2;
	/**
	 * 指示器长度为tab的总长度的1/3
	 */
	public static final int INDICATOR_LENGTH_MODE_3 = 3;
...
//		mDividerPaint.setColor(Color.parseColor("#1a9df5"));
	 ...
	 * 设置指示器的长度
	 */
	public void setIndicatorLength(int mode){
		indicatorMode = mode;
	}
	...
	private void addTextTab(final int position, CharSequence title) {
		...
		if (pressedDrawable == null) {
			// 按下默认按下状态是一个透明的白色背景
			pressedDrawable = new ColorDrawable(0x00ffffff);
			normalDrawable = null;
...
//				canvas.drawLine(child.getRight(), mDividerPadding, child.getRight(), mContentHeight - mDividerPadding, mDividerPaint);
	...
	private void checkAndcalculate() {
		...
		if (mIndicatorLeft < firstTab.getLeft()) {
			mIndicatorWidth = firstTab.getWidth() / indicatorMode;
			mIndicatorLeft = firstTab.getLeft() + getLeftSize();
		}
		...
		if (mIndicatorLeft > lastTab.getLeft()) {
			mIndicatorWidth = lastTab.getWidth() / indicatorMode;
			mIndicatorLeft = lastTab.getLeft() + getLeftSize();
		}
...
//		mIndicatorLeft = (int) (tab.getLeft() + tab.getWidth() * offset + 0.5) + mIndicatorWidth / 2;
		int rightPosition = position + 1;
		if (offset > 0 && rightPosition < mTabCount) {
			View rightTab = getChildAt(rightPosition);
			mIndicatorWidth = (int) ((tab.getWidth() * (1 - offset) + rightTab.getWidth() * offset + 0.5)) / indicatorMode;
		} else {
			mIndicatorWidth = tab.getWidth() / indicatorMode;
		}
		mIndicatorLeft = (int) (tab.getLeft() + tab.getWidth() * offset + 0.5) + getLeftSize();
	...
	private int getLeftSize(){
		return indicatorMode == 1 ? 0 : (mIndicatorWidth / indicatorMode);
	}
