...
public class ImageShareTask {
    ...
    private ImageUploadRequstObject request204;
    ...
    public ImageShareTask(ImageUploadRequstObject request) {
        this(request, null);
    }

    public ImageShareTask(ImageUploadRequstObject request, Callback callback) {
        this.callback = callback;
        this.request204 = request;
        create204();
    }

    public ImageShareTask(ImageShareRequestObject request) {
        this(request, null);
    }

    public ImageShareTask(ImageShareRequestObject request, Callback callback) {
        this.callback = callback;
     ...
     * 初始化204
     */
    private void create204() {

        this.pk_id = request204.getPk_id();
        this.member_id = request204.getMember_id();
        this.files = request204.getData();
        this.team_id = request204.getTeam_id();
    ...
    private void create() {
        ...
        // LPDSStorageUtil.getLpdsUploadimage().getPath()
        try {
            uploadManager = new UploadManager(new FileRecorder(SYSJStorageUtil.getSysjUploadimage().getPath()),
                    new KeyGenerator() {
                        @Override
     ...
     * 图片上传凭证204
     */
    private void preparing204() {

        // 图片上传凭证
        RetPhotoKeyAndToken204Entity entity =
                DataManager.UPLOAD.retPhotoKeyAndToken204(member_id, String.valueOf(files.size()));
    ...
    private void preparing() {
        ...
        // 图片上传凭证
        PhotoRetPhotoNameAndTokenEntity entity =
                DataManager.UPLOAD.photoRetPhotoNameAndToken(member_id, title, String.valueOf(files.size()));
        Log.i(TAG, "entity=" + entity);
        if (entity != null && entity.isResult()) {
            token = entity.getData().getToken();
            if (entity.getData() != null && entity.getData().getToken() != null) {
     ...
     * 七牛上传
     */
    private void qiniu(final int number, final String token, final String key, final String path, final boolean isMatchResult) {
            ...
            public void complete(final String key, ResponseInfo respInfo, JSONObject jsonData) {
                ...
                if (completeNumber == files.size() && newKeys.size() > 0) {
                    ...
                    handler.post(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            completing(number, isMatchResult);
                        }
                    });
                }
            }
     ...
     * 图片上传回调
     */
    private void completing(final int number, final boolean isMatchResult) {
        // 图片上传回调
        BaseResponseEntity entity;
        if (isMatchResult) {
            entity = DataManager.UPLOAD.saveEventResult204(pk_id, newKeys, team_id);
        } else {
            entity = DataManager.UPLOAD.photoSavePhoto(member_id, game_id, newKeys, title, description);
        }
