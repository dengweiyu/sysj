...
import com.li.videoapplication.mvp.adapter.HomeMultipleAdapterNew2;
...
public class HomeLazyColumnFragment2 extends TBaseFragment
    ...
    private HomeMultipleAdapterNew2 mAdapter;

    private List<MyHomeModuleEntity.ADataBean> mData; //获取的总数据
    ...
    public static HomeLazyColumnFragment2 newInstance(String columnId, boolean isNeedLoadData,
                                                      int delayTime, boolean isLazyLoad) {
        final HomeLazyColumnFragment2 fragment = new HomeLazyColumnFragment2();

        Bundle bundle = new Bundle();
        bundle.putBoolean("is_need_load_data", isNeedLoadData);
        bundle.putString("column_id", columnId);
        bundle.putBoolean(HomeLazyColumnFragment2.INTENT_BOOLEAN_LAZY_LOAD, isLazyLoad);
...
//                super.setBaseRootView(layout);
    ...
    private void loadView() {
        ...
        if (mAdapter == null) {
            mAdapter = new HomeMultipleAdapterNew2(this, mData);
        }
        ...
        mAdapter.setSpanSizeLookup(new BaseQuickAdapter.SpanSizeLookup() {
            ...
            public int getSpanSize(GridLayoutManager gridLayoutManager, int position) {
                if (!mData.isEmpty()) {
                    mData.get(position).getSpanSize();
                }
                return 0;
            }
        });
        ...
        if (entity.isResult()) {
            ...
            for (HomeModuleEntity.ADataBean dataBean : entity.getAData()) {
                ...
                if (dataBean.getFlag().equals("hotGame")) {
                    MyHomeModuleEntity.ADataBean myDataBean = new MyHomeModuleEntity.ADataBean();
                    myDataBean.setSpanSize(MyHomeModuleEntity.ADataBean.TITLE_SPAN_SIZE);
                    myDataBean.setItemType(MyHomeModuleEntity.TYPE_HOT_GAME);
                    myDataBean.setList(dataBean.getList());
                    mData.add(myDataBean);
                    continue;
                }
                if (dataBean.getFlag().equals("guessVideo")) {
                    MyHomeModuleEntity.ADataBean myDataBean = new MyHomeModuleEntity.ADataBean();
                    myDataBean.setSpanSize(MyHomeModuleEntity.ADataBean.TITLE_SPAN_SIZE);
                    myDataBean.setItemType(MyHomeModuleEntity.TYPE_GUESS_YOU_LIKE_TITLE);
                    mData.add(myDataBean);
//                    if (AppConstant.SHOW_DOWNLOAD_AD) {//普通渠道，替换广告
//                        replaseGDT(HomeMultipleAdapterNew2.changeVideoImageType(dataBean.getList()), GUESSVIDEO_CHANGE);
//                    }
                    for (int i = 0; i < dataBean.getList().size(); i++) {
                        mData.add(new MyHomeModuleEntity.ADataBean(dataBean.getList().get(i), MyHomeModuleEntity.ADataBean.VIDEO_SPAN_SIZE, MyHomeModuleEntity.TYPE_GUESS_YOU_LIKE));
                    }
                    continue;
                }
                if (dataBean.getFlag().equals("hotMember")) {
                    MyHomeModuleEntity.ADataBean myDataBean = new MyHomeModuleEntity.ADataBean();
                    myDataBean.setSpanSize(MyHomeModuleEntity.ADataBean.TITLE_SPAN_SIZE);
                    myDataBean.setItemType(MyHomeModuleEntity.TYPE_HOT_NARRATE);
                    myDataBean.setList(dataBean.getList());
                    mData.add(myDataBean);
                    continue;
                }
                if (dataBean.getFlag().equals("common")) {
                    if (dataBean.getMore_mark().equals("sysj_original")) {
                        MyHomeModuleEntity.ADataBean myDataBean = new MyHomeModuleEntity.ADataBean();
                        myDataBean.setSpanSize(MyHomeModuleEntity.ADataBean.TITLE_SPAN_SIZE);
                        myDataBean.setItemType(MyHomeModuleEntity.TYPE_SYSJ_VIDEO_TITLE);
                        mData.add(myDataBean);
                        for (int i = 0; i < dataBean.getList().size(); i++) {
                            mData.add(new MyHomeModuleEntity.ADataBean(dataBean.getList().get(i), MyHomeModuleEntity.ADataBean.VIDEO_SPAN_SIZE, MyHomeModuleEntity.TYPE_SYSJ_VIDEO));
                        }
                        continue;
                    } else {
                        MyHomeModuleEntity.ADataBean myDataBean = new MyHomeModuleEntity.ADataBean();
                        myDataBean.setSpanSize(MyHomeModuleEntity.ADataBean.TITLE_SPAN_SIZE);
                        myDataBean.setItemType(MyHomeModuleEntity.TYPE_VIDEO_GROUP_TITLE);
                        myDataBean.setTitle(dataBean.getTitle());
                        mData.add(myDataBean);
                        for (int i = 0; i < dataBean.getList().size(); i++) {
                            mData.add(new MyHomeModuleEntity.ADataBean(dataBean.getList().get(i), MyHomeModuleEntity.ADataBean.VIDEO_SPAN_SIZE, MyHomeModuleEntity.TYPE_VIDEO_GROUP));
                        }
                        continue;
                    }
                }
                if (dataBean.getFlag().equals("gamerVideo")) {
                    MyHomeModuleEntity.ADataBean myDataBean = new MyHomeModuleEntity.ADataBean();
                    myDataBean.setSpanSize(MyHomeModuleEntity.ADataBean.TITLE_SPAN_SIZE);
                    myDataBean.setItemType(MyHomeModuleEntity.TYPE_GAMER_VIDEO_TITLE);
                    myDataBean.setTitle(dataBean.getTitle());
                    mData.add(myDataBean);
                    for (int i = 0; i < dataBean.getList().size(); i++) {
                        mData.add(new MyHomeModuleEntity.ADataBean(dataBean.getList().get(i), MyHomeModuleEntity.ADataBean.VIDEO_SPAN_SIZE, MyHomeModuleEntity.TYPE_GAMER_VIDEO));
                    }
                }
            }
